...
// Description :  with semaphore, only one process could  	    //
//                enter the critical section. And other process //
//                just wait till process that entered the       //
...
{
    int val;
    struct semid_ds *buf;      // struct semaphore (time,queue,pointer of first array,permission)
...
//==============================================================//
// Input: 	int -> semid					//
// Description:	semaphore number				//
//								//
// output: 	void 						//
// Description: -						//
//								//
// Purpose : 	if processer call func 				//
...
void v(int semid)
{
    ...
    if ((semop(semid, &vbuf, 1)) == -1) // set v operation
    {
...
bool subProcess(char *url, char *Cache_Dir, FILE *log)
{
    ...
    if (length > 3)
    {
        ...
        if (!pDir)
        {
            ...
            p(semid);  // critical section으로 한개의 프로세스가 들어감
            //sleep(10);
...
int main(int argc, char const *argv[])
{
    ...
    if((semid=semget((key_t)39999,1,IPC_CREAT|0666))==-1)//setting the semid
    {
        perror("semget failed");
        exit(1);
    }
    arg.val=1;//value of semaphore
    if((semctl(semid,0,SETVAL,arg)== -1)){//setting the semid's function
